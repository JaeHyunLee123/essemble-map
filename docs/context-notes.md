...
# Phase 11: 합주실 상세 및 제보 폼 아키텍처 개선 (Refactoring) 컨텍스트 노트

## 결정 사항 및 이유

### 1. UI 컴포넌트의 초경량화 및 도메인 단위의 씸(Seam) 캡슐화
- 복잡하게 컴포넌트 내부에 결합되어 데이터 로딩 라이프사이클을 통제하고 있던 상세 정보 쿼리(`useStudioDetail`)와 제출용 뮤테이션(`useStudioSubmit`) 로직을 각각의 씸 뒤로 격리하였습니다.
- 폼 컴포넌트는 오직 뷰 표현 및 사용자 폼 검증(Zod)에만 집중하도록 구조적 책임을 분리하여 복잡도를 줄였습니다.
- 이로써 UI 마크업 레이아웃과 데이터 레이어의 의존관계가 단일 Seam 인터페이스로 간결하게 수렴되었습니다.
