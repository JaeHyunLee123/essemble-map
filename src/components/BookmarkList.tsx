// 사용자가 북마크한 합주실 목록을 카드 그리드 형식으로 렌더링하는 컴포넌트입니다.
"use client";
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-sm text-zinc-400">북마크한 합주실을 불러오는 중입니다...</p>
      </div>
    );
  }
  ...
  if (bookmarks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-zinc-800/80 rounded-2xl bg-zinc-900/10 backdrop-blur-sm max-w-lg mx-auto">
        ...
        <h3 className="font-semibold text-lg text-zinc-200 mb-1">북마크한 합주실이 없습니다</h3>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto p-1">
      {bookmarks.map((studio) => (
        <div
          key={studio.id}
          className="group relative overflow-hidden rounded-2xl bg-zinc-900/60 backdrop-blur-xl border border-zinc-800/80 p-5 transition-all duration-300 hover:border-zinc-700/80 hover:shadow-xl hover:shadow-emerald-950/5 hover:-translate-y-0.5"
        >
          {/* 상단 장식 효과 */}
          ...
          <div className="flex flex-col h-full justify-between gap-4">
            <div>
              <div className="flex justify-between items-start gap-2 mb-2">
                <h4 className="font-bold text-zinc-100 text-base group-hover:text-emerald-400 transition-colors duration-300 leading-tight">
                  {studio.name}
                </h4>
                <button
                  onClick={() => handleRemoveBookmark(studio.id)}
                  title="북마크 해제"
                  className="p-1.5 rounded-lg bg-zinc-950/80 border border-zinc-800 text-zinc-400 hover:text-rose-400 hover:border-rose-500/20 hover:bg-rose-500/5 transition-all outline-none cursor-pointer"
                >
            ...
            <div className="flex items-center justify-between border-t border-zinc-800/60 pt-3.5 mt-auto">
              ...
              {studio.mapUrl && (
                <a
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-zinc-950 border border-zinc-800 hover:border-emerald-500/30 hover:bg-emerald-500/5 text-zinc-300 hover:text-emerald-400 transition-all outline-none"
                >
                  지도 보기
