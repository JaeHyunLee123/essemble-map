...
/**
 * 합주실 제보 등록 (POST /api/studios/submit)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // 4. DB에 제보 정보 등록 (images는 빈 배열, status는 pending 고정)
    ...
    return NextResponse.json(
      successResponse({
        ...
        message: "합주실 제보가 정상적으로 접수되었습니다. 관리자 승인 후 지도에 표시됩니다.",
      }),
      { status: 201 }
    );
  } catch (error) {
    console.error("Submit studio error:", error);
    return NextResponse.json(
